...
import { SVGImage } from "components/atoms";
...
const CommentIcon: React.FC<IProps> = ({ isLike, handleClickLike, handleClickComment }) => {
    const path = {
        liked:
        ...
        <Container>
            <IconContainer onClick={handleClickLike} style={{ marginRight: "16px" }}>
                <SVGImage
                    height="24"
                    width="24"
                    fill={isLike ? "#ed4956" : undefined}
                    d={isLike ? path.liked : path.unliked}
                />
            </IconContainer>
            <IconContainer onClick={handleClickComment}>
                <SVGImage height="24" width="24" d={path.comment} />
            </IconContainer>
        </Container>
    );
};
